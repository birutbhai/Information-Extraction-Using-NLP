...
if __name__ == "__main__":
    ...
    for file, text in file_dict.items():
        ...
        for line in lines:
            try:
                ...
                for sentence in sentences:
                    ...
                    for word in doc:
                        dep_list.append([word.text, word.dep_, word.shape_, word.is_alpha, word.is_stop, word.head.text, word.head.pos_, [child for child in word.children]])
                    lemma_list = list()
                    for token in tokens:
                        #print(token[0])
        ...
        dependenc_parser_dict.update({file:dep_list}) 
...
for key, value in file_dict.items():
    if key is not "":
        ...
        o_file.write("\n")
for key, value in dependenc_parser_dict.items():
    if key is not "":
        o_file.write(str(key+": " + str(value)))
        o_file.write("\n")
o_file.close()
